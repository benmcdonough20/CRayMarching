...
struct vec {
	...
	vec space_distortion(){ //let have some fun! this function will act on ray, tricking the object's max_dist and normal function into thinking the ray is in a different location
...
class box: public scene_object{
	public:
		...
		double max_dist(vec ray) override {
			...
			return (vec(x,y,z)-vec(w,h,l)).arg();
		}
		vec normal(vec ray) override {
			double x = std::abs(ray.x-loc.x);
			double y = std::abs(ray.y-loc.y);
...
light* lights[NUM_LIGHTS] = {
	...
	new light(vec(20,20,20), vec(30,30,30), vec(-0,5,0), 0),
};

scene_object* objs[NUM_OBJECTS] = {
	new sphere(vec(-5,0,7), 2, sphere_mat), 
	new ground_plane(vec(0,-2,0), ground_mat)
};
...
bool march(vec direction, vec ray, bool secondary = false){
	int step = 0;
	while(step < MAX_NUM_STEPS && ray.arg() < MAX_DIST){
		int index = 0;
		...
		if( min_dist < MIN_DIST ) {
			...
			for ( int l = 0; l < NUM_LIGHTS; l ++){
				...
				if(lights[l]->type == 0) {
					falloff = pow((lights[l]->loc-ray).arg(), -2)*10;
